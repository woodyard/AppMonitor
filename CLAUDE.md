...
- Shipped: v1.1.15 on GitHub Releases (2026-09-22; 1.1.14 the same day, its take-over failed on a device with two registered versions of the package): browser admin console, WPF local mode deprecated, stale scheduled updates pruned, web-source validation in both consoles, per-app `WingetReplaceOnMismatch` (winget uninstall + install on the 0x8A15008E "install technology is different" refusal; Oh My Posh on H-PARALLELSVM is the case). Devices pick it up by self-update.
