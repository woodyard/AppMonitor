...
// =====================================================================================================================
// Ported from src/Arkimentum.AppMonitor.Core/Configuration/SettingsDocument.cs. This is the ONE place in the API where
// organization configuration is validated: every write path (PUT /admin/organizations/{id}/config and the seeding done
// by the deploy script) goes through SettingsDocument.Validate(). The rules are asserted against Core's implementation
// in SettingsValidationTests using the same sample values.
// =====================================================================================================================

/// <summary>
/// A distributable settings profile: the raw values of one registry layer, without defaults.
/// Values are stored in natural JSON types (bool, number, string, string[]) and validated against
/// <see cref="SettingsSchema"/>.
/// </summary>
public sealed class SettingsDocument
{
    ...
    /// <summary>Compact form used for storage and for hashing into a ConfigVersion (stable, sorted key order).</summary>
    private static readonly JsonSerializerOptions Compact = new(JsonSerializerDefaults.General)
    {
        WriteIndented = false,
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull,
        PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
    ...
    public string ToCompactJson() => JsonSerializer.Serialize(this, Compact);
    ...
    /// <summary>Re-creates the dictionaries with the case-insensitive comparer (model binding loses it).</summary>
    public void Normalize()
    {
        Global = new SortedDictionary<string, SettingValue>(Global, StringComparer.OrdinalIgnoreCase);
        Apps = new SortedDictionary<string, SortedDictionary<string, SettingValue>>(
            Apps.ToDictionary(k => k.Key, v => new SortedDictionary<string, SettingValue>(v.Value, StringComparer.OrdinalIgnoreCase)), StringComparer.OrdinalIgnoreCase);
    }
...
/// <summary>A single setting value with lenient conversions between the JSON and text representations.</summary>
