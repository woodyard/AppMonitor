...
// =====================================================================================================================
// Ported from src/Arkimentum.AppMonitor.Core/Configuration/SettingsSchema.cs. The API cannot reference Core (net10.0-
// windows), so the catalogue is duplicated here - minus the registry-specific members (RegistryValueKind). The names,
// kinds, defaults and ranges must match Core exactly; SettingsSchemaParityTests asserts that against the Core types.
// =====================================================================================================================
...
public enum SettingKind
{
    ...
    Int,
    String,
    /// <summary>A string that may contain environment variables (stored as REG_EXPAND_SZ on the device).</summary>
    Path,
    /// <summary>Comma-separated integers, e.g. deferral options in minutes.</summary>
    IntList,
    /// <summary>List of strings (stored as REG_MULTI_SZ on the device).</summary>
    StringList,
    /// <summary>String restricted to <see cref="SettingDefinition.Choices"/>.</summary>
    Choice,
}

/// <summary>Describes one registry value the agent understands.</summary>
...
public static class SettingsSchema
{
    ...
    [
        ...
        new("AgentUpdateFeedUrl", SettingKind.String, CategoryCloud, "Agent update feed URL", "GitHub latest-release API URL or a manifest.json URL. Enter 'cloud' to use the cloud API's release mirror instead. Default: the official AppMonitor releases on GitHub.", "https://api.github.com/repos/woodyard/AppMonitor/releases/latest", Advanced: true),
