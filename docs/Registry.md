...
| Value | Type | Default | Meaning |
| --- | --- | --- | --- |
| `WingetId` | SZ | catalog value | winget package id. **Required** for `Source=winget`; the application is skipped with a warning without it. It is a hint that takes precedence, not the only id the agent will use: when the configured id is not installed, the scan resolves the id from winget's own listing - the row whose name passes the application's identity rule (`DetectDisplayNameRegex`, else `DisplayName` as a whole-word prefix) - and prefers the id `winget upgrade` names for that install (e.g. `Adobe.Acrobat.Reader.32-bit` for `Adobe.Acrobat.Reader.64-bit`, `Google.Chrome.EXE` for a per-user Chrome, `Mozilla.Firefox.MSIX` for the Firefox MSIX build). Alternatives may still be listed separated by `;` (or `|` inside an `AppList` value), e.g. `Mozilla.Firefox;Mozilla.Firefox.MSIX`; they are optional hints, tried in order before the name rule. See [Troubleshooting](Troubleshooting.md#an-install-asked-for-administrator-rights). |
