...
namespace Arkimentum.AppMonitor.Versioning;

/// <summary>
/// Lenient version comparison for vendor version strings ("26.02.00.0", "153.0.8010.37", "v8.7.1", "1.2.3-beta", "2024.1").
/// Numeric segments compare numerically; textual segments compare ordinal-ignore-case; a numeric segment ranks above text
/// so "1.2.3" &gt; "1.2.3-beta". Missing trailing segments count as zero ("1.2" == "1.2.0").
/// </summary>
    ...
    private static List<Token> Tokenize(string v)
    {
        ...
        foreach (var part in SeparatorRegex().Split(v))
        {
            ...
            foreach (Match m in TokenRegex().Matches(part))
            {
                ...
                if (char.IsDigit(s[0]))
                {
                    // very long numerics (e.g. build timestamps) still compare numerically via decimal
